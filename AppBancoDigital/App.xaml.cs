...
namespace BancoAPITest
{
	...
	{
		private static int IdCorrentista = 11;
		private delegate void TransacaoData(Correntista remetente, Correntista destinatario, TransacaoCompleta transacao);
		private static event TransacaoData OnTransacaoRecebida;

		private async static Task Main(string[] args)
		{
			OnTransacaoRecebida += RegistrarTransacao;
			await ListenForTransacoes(IdCorrentista);
		}

		private static async Task ListenForTransacoes(int id_correntista)
		{
			List<Conta> contas = await DataServiceConta.GetContasByCorrentista(id_correntista);
